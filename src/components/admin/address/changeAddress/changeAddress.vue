<template>
  <div class="wrap" @touchmove.prevent>
    ...
    <div class="box">
      ...
      <div class="addCardTo" id="addCardTo" @click="changeAddressTo">保存</div>
    </div>
...
export default {
  ...
  created(){
    ...
    this.parameter = Object.assign(this.parameter,this.address.Items.filter((ele,index)=>{
      return ele.Id == parseInt(this.Id)
    })[0]);
    this.check.push(this.parameter.Default)
  },
  methods:{
    ...
    changeAddressTo(){
      ...
      addNewAddress(parameter).then((response)=>{
        if(!response.data.res){
          for(let i=0;i<this.address.Items.length; i++){
            if(parameter.Default == "Y"){
              this.address.Items[i].Default = "N";
            }
            if(this.address.Items[i].Id == parseInt(parameter.Id)){
              this.address.Items.splice(i,1,{
                City:parameter.City,
  ...
  watch:{
    check:function (value) {
      this.parameter.Default = value[1]||"N"
    }
  },
